...
**The jar reaches disk when the CDP connection closes, and only then.** Three runs against the same engine isolated it (`scratch/flush-ab.ts`): with a cookie set and the connection still open, `cookies.json` was still absent after 3 s; with the connection closed and the process left running, the file appeared **211 ms** later, holding the cookie; with the process killed straight after the set, the file never appeared. This is why the plugin's stop path had to change: `stopEngine` closed the connection without awaiting it and killed the child in the same tick, so every stop lost the session. It now awaits the close and settles `SESSION_FLUSH_MS` (300 ms) before the kill, and the session self check proves the cookie survives a restart (`scripts/session-selfcheck.ts`). The class of mistake is worth noting: the first version of that check failed with `0 !== 1` and looked like a persistence problem, while the real defect was in the plugin's shutdown ordering.
