...
Reconciled on 2026-09-21, during spec 0006's verify pass: a third trigger now raises the death verdict. The two recorded triggers, the process exit and the WebSocket close, both still fire, but they arrive asynchronously, so a call that hit the already dead socket failed twice before the verdict landed. Two code defects produced that: the mapper's engine down pattern did not match the raw transport codes a killed engine writes (so the caller saw "write ECONNRESET" instead of a plain message), and classification was not idempotent, so an action op re-wrapped the plain snapshot error and dropped the marker the queue reads. Now the in flight call that hits the transport failure raises the verdict itself, and the state machine reaps a still live child on the way out, so exactly one call fails and the next respawns. This adds a detection path and tightens the timing; it changes neither the chosen option, nor the page loss tradeoff, nor the two listener triggers.